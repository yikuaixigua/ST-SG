...
from data_proc.data_create import data_create
from finetune_utils.predict import predict_changemap
...
def main(args):
    ...
    patch_size = 11
    k = 35
    data_create(cfgs['Data']['image1'], cfgs['Data']['image2'], cfgs['Data']['label'], segsize, patch_size, k)

    # 加载GPU
    device = gpu_setup(cfgs['GPU']['use'], 1)
    ...
    # 读取数据集
    dataset = ST_DatasetLoad('data' + str(segsize) + str(patch_size) + str(k) + '.pkl', train_ratio=cfgs['Train']['finetune_train_ratio'], shuffle=True)
    ...
    for epoch in range(epochs):
        ...
        if finetune_train_loss < best_loss:
            ...
            save_state_dict(
                path=os.path.join(save_dir, 'best_finetune'+ str(segsize)+ str(patch_size) + str(k)  + '.pkl'),
                model=model,
                cfgs=args,
                epoch=epoch,
                optim=optimizer,
    ...
    # 预测测试数据变化检测结果
    fintunepath = 'finetune_checkpoints/best_finetune'+ str(segsize) + str(patch_size) + str(k) + '.pkl'
    predict_changemap(cfgs['Data']['image1'], cfgs['Data']['image2'], cfgs['Data']['label'], segsize, patch_size, k, fintunepath, device)
